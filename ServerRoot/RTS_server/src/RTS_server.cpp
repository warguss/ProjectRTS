...
int main(int argc, char* argv[]) {
	...
	while(true)
	{
		...
		int clientSock = accept(serverSock, (sockaddr*)&serverAddr, (socklen_t*)servAddrSize); 
		if ( clientSock <= 0 )
		{		
		...
		int32_t readSize;
		if ( (readSize = recv(clientSock, buffer, (int32_t)READ_BUFSIZE, MSG_DONTWAIT)) > 0 )
		{
			 ...
			 **************************************/
			int32_t writeSize;
			if ( (writeSize = send(clientSock, buffer, (size_t)readSize, MSG_DONTWAIT) ) < 0 )
			{
				LOG("Error Send");
			} 
